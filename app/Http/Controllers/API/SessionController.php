...
class SessionController extends Controller
{
    ...
    public function getStudentAcademicSession($student_id = null)
    {
        ...
        $sessions = Session::join('student_session', 'sessions.id', '=', 'student_session.session_id')
            ...
            ->groupBy('sessions.id', 'sessions.session', 'sessions.status')
            ->orderBy('sessions.id')
            ->select('sessions.id', 'sessions.session', 'sessions.status')
            ->get()
            ->toArray();
        return response()->json(['data' => $sessions], 200);
    }
    ...
    public function add(Request $request)
    {
        try {
            ...
            $data = $request->validate([
                ...
                'payload' => 'nullable|string', // Changed to nullable
                'last_activity' => 'nullable|integer', // Changed to nullable
            ]);
            ...
            if (isset($data['id'])) {
                ...
                $session = Session::create($data); // No manual id generation needed
